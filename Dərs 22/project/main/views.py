...
def telefon(request):
    ...
    if request.method == 'POST':
        ...
        foto = request.FILES['foto']
        fs = FileSystemStorage()
        file = fs.save(foto.name,foto)
        file_url = fs.url(file)
        
        model = request.POST['model']
        ram = request.POST['ram']
        
        data = Telefon(img = file_url,model = model,ram=ram )
        data.save()
...
def axtar(request):
    a = request.POST['sorgu']
    data = Telefon.objects.filter(Q(model__contains=a) | Q(ram__contains = a) | Q(dioqonal__contains = a))
    
    
    return render(request,'main/axtar.html',{'data':data})


def etrafli(request,id):
    
    telefon = Telefon.objects.filter(id = id)
    
    return render(request,'main/etrafli.html',{'telefon':telefon})
...
def shop(request):
    ...
    return render(request,'main/shop.html',{'data':data,'telefon':telefon})
...
def item(request):
    ...
    if request.method == 'POST':
        ...
        cat = Category.objects.get(id=request.POST['item'])
